...
async function plotGraph() {
    ...
    const modelCumOil = parsedData.data.map(row => parseFloat(row['Predicted Cumulative GAS (BCM)']));
    const actualCumOil = parsedData.data.map(row => parseFloat(row['GRPC Cumulative GAS (BCM)']));
    const modelOilDiff = parsedData.data.map(row => parseFloat(row['Predicted Yearly GAS rate (BCM/yr)']));
    const actualOilDiff = parsedData.data.map(row => parseFloat(row['GRPC Yearly GAS rate (BCM/yr)']));
